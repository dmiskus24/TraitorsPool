...
function draftView(){
  ...
  <div class="contestants">${CONTESTANTS.map(([name,job])=>{ const picked=pm[name]; return `<article class="card ${picked?'picked':''}"><button class="contestant-info-trigger avatar-trigger" data-contestant="${esc(name)}" aria-label="View ${esc(name)} profile">${avatarMarkup(name,'avatar')}</button><div class="cardbody"><button class="contestant-name contestant-info-trigger" data-contestant="${esc(name)}"><h3>${esc(name)}</h3><p>${esc(job)}</p></button>${picked?`<span class="drafted">Drafted by ${esc(picked.owner)}</span>`:`<button class="pickbtn" data-pick="${esc(name)}" ${cp!==state.selectedPlayer?'disabled':''}>Draft</button>`}</div></article>`}).join('')}</div></div>
